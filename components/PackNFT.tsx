...
export const PackNFTCard = ({ listingId, tokenId }: Props) => {
  ...
  const { contract: packContract } = useContract(PACK_ADDRESS);
      ...
      {!loadingNFT && !loadingPackListings ? (
        <div className={styles.shopPack}>
          ...
          <div className={styles.packInfo}>
            ...
            <p>
              Cost:{" "}
              {packListings && packListings.currencyValuePerToken.displayValue}{" "}
              {packListings && ` ` + packListings.currencyValuePerToken.symbol}
            </p>
            <p>Supply: {packListings && packListings.quantity}</p>
            {!address ? (
              <p>Login to buy</p>
            ) : (
              <Web3Button
